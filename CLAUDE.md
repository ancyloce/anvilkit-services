...
- Only `docs/archive/`, `outputs/`, `logs/`, `.local/` and build caches are gitignored. Inputs added by a task are untracked until the user stages them, and `.local/` (dev foundation credentials, snapshots, the preserved legacy worktree) exists only in the primary checkout; a Git worktree therefore lacks them. Run tasks in the primary checkout and edit files there. `contracts/` and `services/agent/api` are their own repositories (see [architecture](docs/architecture/architecture.md#repositories)): changes inside them are changes of those repositories.
...
- Read-only checks: `sh tools/verification-env.sh --static` (pinned CPython 3.12 venv under `.local/`, then the chain), or with `.local/verification-venv/bin/python`: `tools/check-docs.py`, `tools/check-contracts.py`, `tools/check-source-export.py`, `tools/generate-contracts.py --check`, `tools/run-verification.py --static`; the contracts repository alone with `sh tools/verification-env.sh` inside `contracts/`; the API alone with `GOWORK=off` in `services/agent/api`. `generate-contracts.py` without `--check` rewrites the checked-in contract consumers and Control's sqlc output; `deploy/dev/up.sh` starts containers and a kind cluster and migrates the dev database forward; `deploy/dev/api-chart.sh install` deploys the API into that cluster.
