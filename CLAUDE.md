...
- Go (from each service clone after its module exists): `GOWORK=off go build -mod=readonly ./...`, `GOWORK=off go test -mod=readonly ./...`; scope changes with `go list -deps`. No Go `project.json` wrappers.
...
- Component fixture: `node tools/component/hero-fixture.mjs all` pins the fixed Hero fixture, runs the component repository's protected commands from the Studio root and builds the npm and browser outputs; `python3 tools/run-verification.py --only hero-host` proves the host ABI in a built Studio checkout ([task record](docs/tasks/0004-development-plan-s0-s1-tasks.md)).
...
| `anvilkit-job-access-proxy` | `jobs/shared/access-sidecar` | Go per-job sidecar. Holds the job scope credential; allowlisted relay to Model Proxy and the artifact adapter. Path per owner decision 2026-09-12 (submodule repository `anvilkit-job-access-sidecar`, superseding `jobs/shared/access-proxy`); the unit name is unchanged |
...
Other planned paths: `internal/integrations/pagix` (typed guarded `PagixPort`), each service's `internal/contracts` and consumed `packages/contracts-ts` outputs (generated), `contracts/{values,proto,openapi,jobs,events,catalog,definitions,actions,telemetry}`, `workflows/definitions/<family>`, `packages/{component-toolchain,job-protocol}`, `deploy/<environment>`, `releases`, `docs/architecture`, `docs/design`, `docs/architecture/{plans,audits}`, `tools`. Temporal, PostgreSQL, Kubernetes/gVisor/CNI and artifact storage are infrastructure, not services.
...
- **Every physical paid request needs its own `AdmitAndClaimModelCall`.** Only the first successful claim returns `dispatchAllowed=true`; duplicates, including same-owner retries, never grant another send. Unknown dispatch or usage retains exposure; timeouts, TTLs, lease expiry or process loss never prove zero cost or authorize redispatch. Trusted not-sent evidence terminates the original send authority; a replacement uses a new `callId` with a `supersedes` link and fresh admission against the current fence, deadline and budget, and `unknown` is never treated as unsent. SDK/transport/provider retries and compaction stay disabled unless each physical call is intercepted and admitted. An unset platform daily cap denies paid admission.
- **Credentials stay isolated.** Provider keys live only in Model Proxy; the job scope token only in the sidecar. Candidate jobs have no provider, production business or npm credentials. Check current Attempt, physical instance, lease, execution/recovery generation and deadline at every consequential boundary; a valid token alone never grants execution. Every build, test or SSR process that runs candidate code is untrusted and low-privilege, with no verdict-write and no submission access; no trusted supervisor, observer or finalizer imports a candidate module. The job container carries only `SETUID`, `SETGID` and `SETPCAP`, and each candidate process is dropped fail-closed to empty supplementary groups, candidate UID/GID and empty capability sets with `no_new_privs`.
- **Durable intake precedes remote mutations.** Initial queue waiting holds no source lease or commercial reservation. Bootstrap order: execution permit, recheck scope and original source revision, fenced lease, funding, then paid planning/coding. The active deadline starts at first permit and never resets. Preparation has no draft and therefore no lease; its lease-free admission is the [DD-02 preparation rule](docs/design/dd-02-control.md#preparation-funding), not a fabricated lease.
...
- **Changes preserve issued effects.** Cancellation, hold/resume and live definition changes fence new dispatch, keep actual costs and effect identities, and reconcile by querying the original identity. The first valid cancel or hold always persists and establishes its fence through a reserved control-command lane, independent of business quotas and idempotent under duplicate commands. Uncertain effects stay visibly `blocked`/reconciling; a timeout never implies success, failure or absence of an effect.
- **Validation is protected.** Generation yields complete source; independent validation plus Pagix candidate registration make it usable. One automatic repair is the pilot default. A trusted observer validates dynamic assertions; candidate-written reports and exit codes never directly establish certification. Authors cannot weaken protected validation, dependency policy, budgets, coverage, generator templates, root locks, CI or the release entry point.
...
- **Two ledgers.** Pagix commercial credits and Agent actual costs are separate; `settlement_pending` never triggers regeneration. Development integration runs on the [DD-02 test-billing amendment](docs/design/dd-02-control.md#test-billing) (owner decision 2026-09-12): test credits are neither commercial credits nor actual-cost budgets, and no real customer is charged.
...
- Respect the scoped CD-01 through CD-05 approval and synchronize its new machine contracts before dependent implementation. Other scopes still require the owning design freeze: DD-01 Workflow/Runner, DD-02 Control/contracts/costs/SSE, DD-03 execution/sandbox, DD-04 component build/preview/certification, DD-05 Studio host bridge/page lock, DD-06 Pagix API mapping. Shared contracts freeze before dependent modules implement in parallel.
